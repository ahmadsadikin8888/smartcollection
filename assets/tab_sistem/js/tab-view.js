...
var tabView_maxNumberOfTabs = 200;	// Maximum number of tabs
...
function showTab(parentId, tabIndex) {
	...
	if (activeTabIndex[parentId] >= 0) {
		...
		img.src = 'https://10.194.51.88/syanida/helpdesk/images/tab_right_inactive.gif';
	...
	img.src = 'https://10.194.51.88/syanida/helpdesk/images/tab_right_active.gif';
...
function rolloverTab() {
	if (this.className.indexOf('tabInactive') >= 0) {
		...
		img.src = 'https://10.194.51.88/syanida/helpdesk/images/tab_right_over.gif';
	}
...
function initTabs(mainContainerID, tabTitles, activeTab, width, height, closeButtonArray, additionalTab) {
	...
	for (var no = 0; no < tabTitles.length; no++) {
		...
		if (closeButtonArray[no]) {
			...
			// closeButton.src = 'https://10.194.51.88/syanida/helpdesk/images/close.gif';
...
function deleteTab(tabLabel, tabIndex, parentId) {
	...
	if (tabLabel) {
		...
		if (index != -1) {
			uri_del = document.getElementById('url_del_' + tabLabel).value;
			// alert(tabLabel);
			if (uri_del) {
				var ajaxIndex = ajaxObjects.length;

				ajaxObjects[ajaxIndex] = new sack();
				ajaxObjects[ajaxIndex].requestFile = uri_del;	// Specifying which file to get
